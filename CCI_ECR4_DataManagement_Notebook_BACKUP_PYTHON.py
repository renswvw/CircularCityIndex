...
#cwd = os.getcwd()
cwd = 'c:/Users/rensw/OneDrive - Universiteit Utrecht/GIMA/Thesis - Buitenland/Thesis/Data/Raw_data/ECR4'
...
# create new folders
#os.makedirs(cwd + '/interim/ECR4_interimdata_07112022')

# create path 
path_rawdata = cwd + '/raw/AQ_2019_shape-files/PM10/interpolated-values/geoTIFF/'
raw_rasterfile = path_rawdata + 'PM10_avg19.tif'
print(raw_rasterfile)

#replace nodata value raster
interim_rasterfile = cwd + '/interim/' + "interim_PM10_avg19.tif"
...
show(rf, ax=ax1, title = 'PM10 by administrative districs')
...
df['Average_PM10_value'] = df['mean']

# Select only the columns with municipality name and value
df = df[['Municipality','Average_PM10_value']]
print(df)

# Data cleaning of useless values
df['Average_PM10_value'] = df['Average_PM10_value'].replace(0, np.nan)
df['Average_PM10_value'] = df['Average_PM10_value'].fillna('NoData')
print(df)
